...
def create_refined_gengar_v2():
    """Refinement 2: Larger head, shorter body, wider mouth"""
    ...
    # === REFINEMENT 2: SHORTER, ROUNDER BODY ===
    # Gengar's body is very compact in official art
    body_blocks = [
        # Core - very round and short
    ...
    # === REFINEMENT 2: MUCH LARGER HEAD ===
    # Head should be ~40% of total height
    head_blocks = [
        # Massive lower head
        (0, 0, 0.15), (0.3, 0, 0.15), (-0.3, 0, 0.15),
        (0, 0.3, 0.15), (0, -0.3, 0.15),
        (0.3, 0.3, 0.15), (0.3, -0.3, 0.15), (-0.3, 0.3, 0.15), (-0.3, -0.3, 0.15),
        # Mid head
        (0, 0, 0.5), (0.3, 0, 0.5), (-0.3, 0, 0.5),
        (0, 0.3, 0.5), (0, -0.3, 0.5),
        (0.3, 0.3, 0.5), (0.3, -0.3, 0.5), (-0.3, 0.3, 0.5), (-0.3, -0.3, 0.5),
        # Upper head
        (0, 0, 0.85), (0.3, 0, 0.85), (-0.3, 0, 0.85),
        (0, 0.3, 0.85), (0, -0.3, 0.85),
        (0.3, 0.3, 0.85), (0.3, -0.3, 0.85), (-0.3, 0.3, 0.85), (-0.3, -0.3, 0.85),
        # Top
        (0, 0, 1.15), (0.25, 0, 1.15), (-0.25, 0, 1.15),
        (0, 0.25, 1.15), (0, -0.25, 1.15),
    ]
    
    for i, pos in enumerate(head_blocks):
        create_box(f"Head_{i}", 0.3, pos, purple_mat)
    
    # === REFINEMENT 2: EARS - Longer, more curve ===
    # Extend further back
    left_ear = [
        (-0.5, 0.1, 1.4), (-0.75, 0.2, 1.7), (-1.0, 0.3, 1.95),
        (-1.25, 0.4, 2.15), (-1.5, 0.5, 2.3), (-1.7, 0.6, 2.4),
    ]
    ...
    back_spikes = [
        ...
        (0, -0.45, 0.7), (0.1, -0.47, 0.65), (-0.1, -0.47, 0.65),
    ]
    for i, pos in enumerate(back_spikes):
        create_box(f"Spike_{i}", 0.14, pos, purple_mat)
    
    # === REFINEMENT 2: WIDER MOUTH ===
    # Takes up more of the face
    mouth_blocks = [
        (0, 0.5, 1.0), (0.25, 0.5, 1.0), (-0.25, 0.5, 1.0),
        (0, 0.52, 0.75), (0.3, 0.52, 0.75), (-0.3, 0.52, 0.75),
        (0, 0.55, 0.5), (0.25, 0.55, 0.5), (-0.25, 0.55, 0.5),
        (0, 0.58, 0.25), (0.2, 0.58, 0.25), (-0.2, 0.58, 0.25),
    ]
    for i, pos in enumerate(mouth_blocks):
        create_box(f"Mouth_{i}", 0.26, pos, dark_purple_mat)
    
    # === TONGUE ===
    tongue_blocks = [
        (0, 0.62, 0.15), (0, 0.65, -0.05),
        (0, 0.68, -0.25), (0, 0.7, -0.4),
    ]
    for i, pos in enumerate(tongue_blocks):
        create_box(f"Tongue_{i}", 0.16, pos, pink_mat)
    
    # === EYES - Smaller, positioned higher ===
    create_box("LeftEyeWhite", 0.14, (-0.25, 0.42, 1.25), white_mat)
    create_box("RightEyeWhite", 0.14, (0.25, 0.42, 1.25), white_mat)
    
    create_box("LeftIris", 0.09, (-0.25, 0.46, 1.3), red_mat)
    create_box("RightIris", 0.09, (0.25, 0.46, 1.3), red_mat)
    
    create_box("LeftPupil", 0.045, (-0.25, 0.48, 1.35), black_mat)
    create_box("RightPupil", 0.045, (0.25, 0.48, 1.35), black_mat)
    
    # === TEETH - More visible ===
    teeth_pos = [
        (-0.28, 0.58, 0.9), (-0.12, 0.6, 0.9), (0.12, 0.6, 0.9), (0.28, 0.58, 0.9),
        (-0.18, 0.62, 0.65), (0, 0.63, 0.65), (0.18, 0.62, 0.65),
        (-0.1, 0.65, 0.4), (0.1, 0.65, 0.4),
    ]
    ...
    bpy.ops.object.delete()
    bpy.ops.object.camera_add(location=(0, -5.5, 0.3))
    camera = bpy.context.active_object
    camera.rotation_euler = (math.radians(85), 0, math.radians(180))
    bpy.context.scene.camera = camera
    
    print("Refined Gengar (v2) created!")
    return camera
